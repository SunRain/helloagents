...
```yaml
测试执行（优先级从高到低）:
  1) task.md 明确列出的测试任务（推荐）
  2) project.md#质量门禁命令（当 task.md 未列出测试任务时）
  3) 项目已有测试套件（兜底）

task.md 测试任务级别标记（与失败处理规则绑定）:
  - [BLOCKER] → ⛔ 阻断性测试（失败必须停下并询问）
  - [WARN] → ⚠️ 警告性测试（失败可继续，但必须标注）
  - [INFO] → ℹ️ 信息性测试（失败记录即可）
  - 未标记 → 默认视为 [WARN]（避免过度阻断）
...
执行留痕（强制）:
  - 必须记录“执行了哪些命令/测试 + 结果摘要”
  - 阶段完成输出（G6.1 模板 A）中的“🔍 质量验证”要点需包含:
    - 门禁/测试通过 X/Y（按级别统计）
    - 失败项列表（仅摘要，不贴全量日志）
```
...
2. 迁移至历史记录目录:
   ...
   - 同名冲突处理（高风险）:
     ...
     - 交互确认模式/执行命令: 必须按 G9.1 输出高风险确认，并提供 3 选 1：
       - [1] 备份并继续（推荐）
       - [2] 覆盖（丢弃旧目录，风险自负）
       - [3] 取消迁移（保留在 plan/，后续手动处理）
     - 推进-全授权: 采用“备份并继续”默认策略，不弹出询问；在总结中标注备份路径与原因

3. 更新历史记录索引: `history/index.md`

4. 迁移后链接校验（ADR/历史索引）:
   - 校验 `history/index.md` 中新增条目的方案包路径是否存在
   - 如本次变更包含 ADR（见 how.md 的“架构决策 ADR”章节）:
     - 校验 `wiki/arch.md` 的 ADR 索引链接是否指向已迁移的 `history/YYYY-MM/.../how.md`
     - 如发现死链/路径不一致: 以实际迁移后的路径为准修正文档；无法自动修复则在总结中标注“⚠ ADR索引链接异常”
