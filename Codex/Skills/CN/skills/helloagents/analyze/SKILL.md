...
使用统一输出格式（G6.1 模板 B），标题行后必须包含状态行。

内容格式: 简要说明(≤2句，含当前评分) + 空行 + 扁平化问题清单(按 G6.5「追问问题清单」)

禁止显示: 评分维度明细、分类标题、文件变更清单；禁止额外“下一步建议”（仅填写模板 B 的 `🔄 下一步` 引导句）
...
────
🔄 下一步: 请按序号逐条回答，或输入"以现有需求继续"跳过追问（可能影响方案质量），或输入"取消"终止
```
...
评分<7分: 立即停止，输出追问，等待响应，不执行阶段B
  ...
  用户选择处理:
    ...
    - "取消":
      - 交互确认模式: 按 G6.1 模板 A（🚫）输出取消
      - 推进模式: 清除 MODE_FULL_AUTH/MODE_PLANNING，按 G6.1 模板 A（🚫）输出取消
      - 取消输出示例: 见 `../../appendix/examples.md`（"需求分析：用户取消"）
  模式处理:
