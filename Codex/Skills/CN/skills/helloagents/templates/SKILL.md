...
> 本节定义“生产就绪”的最小可执行门禁（可复现）。开发实施阶段（P3）应按本表执行并记录结果。

**level 可选值:** `BLOCKER`（阻断） / `WARN`（警告） / `INFO`（信息）

| gate | level | command | 说明 |
|------|-------|---------|------|
| format | [BLOCKER/WARN/INFO] | `[如: npm run fmt]` | [格式化/代码风格] |
| lint | [BLOCKER/WARN/INFO] | `[如: npm run lint]` | [静态检查] |
| typecheck | [BLOCKER/WARN/INFO] | `[如: npm run typecheck]` | [类型检查] |
| test | [BLOCKER/WARN/INFO] | `[如: npm test]` | [单元/集成测试] |
| build | [BLOCKER/WARN/INFO] | `[如: npm run build]` | [构建校验] |

> 如项目不适用某项 gate，请将 command 填写为 `无` 并在说明中解释原因。
...
### 轻量迭代精简模板：plan/ 和 history/ 下的 why.md（精简）

**适用场景：** 路由判定为“轻量迭代”时使用，目标是 **最小可追溯闭环**（可被知识库同步与任务验证消费）。

**强制约束：**
- 必须包含 **核心场景** 章节
- 除“核心场景”外，其余章节允许精简为要点列表
...
## 测试与部署
- **质量门禁:** [引用 project.md#质量门禁命令；如本变更新增门禁命令需在此补充]
- **验证命令:** [可复现的命令列表]
- **验收点:** [关键验证点列表]
- **部署:** [流程/影响范围]
```
...
### 轻量迭代精简模板：plan/ 和 history/ 下的 how.md（精简）

**适用场景：** 路由判定为“轻量迭代”时使用。

**强制约束：**
- 必须包含 **技术方案** 与 **测试与部署**（用于开发实施阶段验证与留痕）
- “架构设计 / ADR / API设计 / 数据模型”等章节按需省略（如不涉及）
...
## 测试与部署
- **质量门禁:** [引用 project.md#质量门禁命令；如不适用写 无]
- **验证命令:** [如: npm test / pytest / mvn test]
- **验收点:** [关键验证点列表]
- **部署影响:** [无/需要发布步骤说明]
```
...
> **级别标记:** `[BLOCKER]`（失败阻断）/ `[WARN]`（失败警告）/ `[INFO]`（失败记录即可）

- [ ] 5.1 [BLOCKER] 执行 `project.md#质量门禁命令` 中的门禁命令（`command=无` 的项跳过并备注原因）
- [ ] 5.2 [BLOCKER] 核心场景测试: [场景名称]（命令: [可复现命令]），验证点: [关键验证点列表]
```
