...
**高风险操作：**
- 文件系统：删除文件/目录、批量修改、移动系统文件、覆盖/替换 history/ 归档（含同名冲突覆盖）
...
**目标:** 避免在同一阶段的关键步骤中途被“插话/新需求”打断导致路由抖动与不可追溯输出。

**锁定范围:**
- 从进入某阶段（需求分析/方案设计/开发实施）的内部步骤开始
- 到该阶段输出 **模板 A/B** 为止（输出后解除锁定）

**锁定中消息处理（按优先级）:**
1. **特殊命令**（~auto/~plan/~exec/~init）: 允许打断，进入“命令确认”流程
2. **当前询问的有效响应**（如评分追问回答、方案序号选择、确认/取消）: 按上下文路径处理
3. **其他输入**（疑似新需求/无关输入）: 暂存为“待处理消息”，在当前阶段输出完成后按序处理；如语义边界不清晰，优先输出“上下文确认”（模板 B）让用户选择继续当前任务/开始新任务

#### 模式迟滞（避免路由抖动）

**默认策略:** 同一任务内 **只升级不降级**（微调 → 轻量迭代 → 标准开发 → 完整研发）。除非用户明确要求降级，否则保持当前或更保守（更完整）路径。

**边界条件处理:**
- 文件数/行数/评分处于边界（如 2 文件、30 行、6-7 分）时，按 G3 输出“不确定因素”并采取更安全路径；必要时用模板 B 让用户确认模式选择
...
- 动作流程:
  ...
  2. 获取项目上下文（按G10快速流程）
  3. 创建轻量方案包（精简版 why.md/how.md/task.md）
  4. 执行代码改动
...
- 轻量方案包规则:
  - 路径: `plan/YYYYMMDDHHMM_<feature>/`
  - 必需文件: `why.md` + `how.md` + `task.md`（允许精简内容）
  - 最小内容约束:
    - `why.md` 必须包含 **核心场景**（供知识库同步提取）
    - `how.md` 必须包含 **技术方案** + **测试与部署**（ADR 章节按需生成）
  - 模板来源: 读取 `templates` Skill 的 **轻量迭代精简模板** 生成（避免自由发挥）
  - 迁移标注: 在 `history/index.md` 中标注“轻量迭代”（可在“类型”列或描述中注明）
- 输出格式:
  ```
  ✅【HelloAGENTS】- 轻量迭代完成
  [状态: MODE=交互确认 | MODE_FULL_AUTH=false | MODE_PLANNING=false | MODE_EXECUTION=false | CREATED_PACKAGE=无 | CURRENT_PACKAGE=无]
