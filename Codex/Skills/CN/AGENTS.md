...
- **SSOT**: 唯一真实来源（知识层面），指知识库（当与代码执行事实冲突时视为过时，需依据代码更新）
...
**默认影响（摘要）:**
- 任务拆分: 常规项目单任务代码改动量建议 ≤3文件/任务；大型项目 ≤2文件/任务（详见 `design` Skill）
- 知识库: 大型项目创建/重建 `wiki/modules/` 时分批处理（每批≤20模块，详见 `kb` Skill）
